  ...
  <div class="home">
    ...
    <div class="container">
      <form v-on:submit.prevent="submit();">
        ...
        <router-link :to="{ name: 'exercise4-products-show', params: { id: product.id } }" class="btn btn-secondary">Cancel</router-link>
      </form>
    </div>
  </div>
</template>
  ...
  methods: {
    submit: function() {
      ...
      axios
        ...
        .then(response => {
          this.$router.push({ name: "exercise4-products-show", params: { id: response.data.id } });
        })
        .catch(error => {
