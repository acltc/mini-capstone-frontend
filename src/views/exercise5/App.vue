<template>
  <div class="app">
    <nav class="navbar navbar-expand-lg navbar-dark bg-dark">
      <router-link :to="{ name: 'exercise5-home'}" class="navbar-brand">{{ appConfig.name }}</router-link>
      ...
      <div class="collapse navbar-collapse" id="navbarSupportedContent">
        <ul class="navbar-nav mr-auto">
          <li class="nav-item active">
            <router-link :to="{ name: 'exercise5-products-index'}" class="nav-link">All products</router-link>
          </li>
          <li v-if="isAdmin()" class="nav-item">
            <router-link :to="{ name: 'exercise5-products-new' }" class="nav-link">New product</router-link>
          </li>
          ...
          <li v-if="!loggedIn()" class="nav-item">
            <router-link :to="{ name: 'exercise5-signup' }" class="nav-link">Signup</router-link>
          </li>
          <li v-if="!loggedIn()" class="nav-item">
            <router-link :to="{ name: 'exercise5-login' }" class="nav-link">Login</router-link>
          </li>
          <li v-if="loggedIn()" class="nav-item">
            <router-link :to="{ name: 'exercise5-logout' }" class="nav-link">Logout</router-link>
          </li>
        </ul>

        <form v-on:submit.prevent="submitSearch()" class="form-inline my-2 my-lg-0">
          <input v-model="searchTerms" class="form-control mr-sm-2" type="search" placeholder="Search" aria-label="Search">
