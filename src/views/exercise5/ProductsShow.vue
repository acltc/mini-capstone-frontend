    ...
    <div class="container">
      ...
      <div class="row">
        <div class="col-md-3">
          ...
          <div class="card mt-4">
            <div class="card-body">
              <form v-on:submit.prevent="submit();" class="mb-4"><label>Quantity:</label> <input v-model="quantity" type="number" class="form-control" /> <input type="submit" class="btn btn-primary mt-2" value="Add to cart!" /></form>
            </div>
          </div>
        </div>
        <div class="col-md-9">
          ...
          <router-link v-if="isAdmin()" :to="{ name: 'exercise5-products-edit', params: { id: product.id } }" class="btn btn-primary mr-1">Edit product</router-link>
          <button v-if="isAdmin()" type="button" class="btn btn-warning mr-1" data-toggle="modal" data-target="#deleteModal">Delete product</button>
          <router-link :to="{ name: 'exercise5-products-index' }" class="btn btn-light">Back to all products</router-link>
        </div>
      </div>
    </div>
